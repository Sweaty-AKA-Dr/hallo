...
system.runSchedule(() => {
  [...world.getPlayers()].forEach((player) => {
    if (!player.hasTag("admin") && Math.abs(player.velocity.x) == 0 && Math.abs(player.velocity.z) == 0)
    ...
    if (getScore(player, "afkTimer") == 5920)
      player.onScreenDisplay.setActionBar("You will be kick in 4...");
    if (getScore(player, "afkTimer") == 5940)
      player.onScreenDisplay.setActionBar("You will be kick in 3...");
    if (getScore(player, "afkTimer") == 5960)
      player.onScreenDisplay.setActionBar("You will be kick in 2...");
